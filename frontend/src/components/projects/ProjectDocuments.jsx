...
export default function ProjectDocuments({ projectId, apiBasePath }) {
  ...
  const loadDocs = async () => {
    try {
      setError('');
      const res = await fetch(`${apiBasePath}/${projectId}/documents`, {
        headers: {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${apiBasePath}/${projectId}/documents/upload`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
