...
export default function EmployeeSidebar() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const data = await getCompanySidebar(); // /api/me/sidebar
 ...
 const moduleItems = (modules ?? []).map((k) => EMP_MODULE_NAV[k]).filter(Boolean);
  ...
  return (
    <aside style={{ width: collapsed ? 80 : 280, transition: 'width .2s ease', background: '#fff',
      borderRight: '1px solid #E5E7EB', height: '100vh', position: 'sticky', top: 0,
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8, flex: 1 }}>
        {loading ? (
          ...
          <div style={{ color: '#6B7280', fontSize: 13, padding: '4px 10px' }}>Không có mục nào.</div>
        ) : navItems.map(it => {
          const Icon = it.icon;
          return (
      ...
      <div style={{ borderTop: '1px solid #F3F4F6', paddingTop: 12 }}>
        <button
          ...
          style={{ width: '100%', background: 'transparent', border: 'none', cursor: 'pointer',
            color: '#4B5563', display: 'flex', alignItems: 'center', gap: 8,
            padding: '8px 10px', borderRadius: 12 }}>
          {collapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}
          <span style={{ display: collapsed ? 'none' : 'inline' }}>{collapsed ? 'Expand' : 'Collapse'}</span>
        </button>
      </div>
    </aside>
  );
