...
const FALLBACK_ACTIVE_KEY = 'company';

export default function SettingsPage() {
  const [active, setActive] = useState(FALLBACK_ACTIVE_KEY);
    ...
    return SETTINGS_NAV
      .map((item) => {
        ...
        const allowedChildren = item.children.filter((child) => {
          if ((child.key === 'modules' || child.key === 'workflow') && !canModules) return false;
          if (child.permission && !can(child.permission.resource, child.permission.action)) return false;
          return true;
        });
  ...
  useEffect(() => {
    const fallback = validKeys.includes(FALLBACK_ACTIVE_KEY)
      ? FALLBACK_ACTIVE_KEY
      : (validKeys[0] || FALLBACK_ACTIVE_KEY);
...
function ChangePasswordForm() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setServerMessage(res.message || 'Password updated successfully');
  ...
  return (
    <form onSubmit={onSubmit} style={{ ...card, padding: 16 }}>
      <div style={{ marginBottom: 6 }}>
        <h2 style={{ margin: 0, color: palette.text }}>Security</h2>
        <div style={{ ...helpStyle, marginTop: 6 }}>
          Change your password. For security reasons, other sessions will be signed out.
        </div>
      </div>
