...
const MIGRATIONS = [
  ...
  /* ── V014: Australian payroll (PAYG Scale 2, SG, leave accrual columns) ── */
  {
    version: 'V014__australian_payroll',
  ...
  /* ── V015: Team attendance-by-date module (per job role via job_role_modules) ── */
  {
    version: 'V015__attendance_history_module',
  ...
  {
    version: 'V016__mobile_pin_security',
    description: 'Store mobile PIN hash and security QA hash on users',
    up: async () => {
      const schema = process.env.DB_NAME;
      const cols = [
  ...
  {
    version: 'V017__mobile_pin_email_recovery',
    description: 'Email recovery code for mobile PIN when security answer is forgotten',
    up: async () => {
      const schema = process.env.DB_NAME;
      const cols = [
